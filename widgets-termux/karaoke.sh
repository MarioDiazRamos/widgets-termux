...
# Wrapper para proyecto karaoke independientesource "$HOME/Proyectos/widgets-termux/lib/util_deps.sh" 2>/dev/null || true

# Lanza ~/Proyectos/karaoke/karaoke.sh con dependencias mínimasensure_storage

ensure_cmd ffmpeg ffmpeg

source "$HOME/Proyectos/widgets-termux/lib/util_deps.sh" 2>/dev/null || truecommand -v yt-dlp >/dev/null 2>&1 || python -m pip install --user yt-dlp >/dev/null 2>&1

ensure_cmd python python

CARPETA_AUDIOS="/storage/emulated/0/Download"

PROYECTO_DIR="$HOME/Proyectos/karaoke"CARPETA_VIDEOS="/storage/emulated/0/Movies/Finales"

SCRIPT="$PROYECTO_DIR/karaoke.sh"CARPETA_DESTINO="/storage/emulated/0/Movies/Finales"

GANANCIA_FRAGMENTO="1.0"

if [ ! -f "$SCRIPT" ]; thenmkdir -p "$CARPETA_DESTINO"

  echo "No se encuentra $SCRIPT. Asegúrate de haber clonado el monorepo en ~/Proyectos."

  exit 1seleccionar_audio() {

fi    local tipo="$1"

    echo "\nBuscando archivos de audio en: $CARPETA_AUDIOS\n"

bash "$SCRIPT" "$@"    if [ "$tipo" = "flac" ]; then
