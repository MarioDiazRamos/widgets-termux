...
def listar_directorio():
    try:
        ...
        for elemento in ruta_segura.iterdir():
            if elemento.is_dir():
                ...
                try:
                    ...
                    mod_timestamp = elemento.stat().st_mtime
                    mod = datetime.fromtimestamp(mod_timestamp).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                except Exception:
                    tam = 0
        ...
        ruta_actual = str(ruta_segura.relative_to(DIRECTORIO_RAIZ))
        return jsonify(
            {
...
def leer_archivo():
    try:
        ruta = request.args.get("path", "")
        ruta_segura = Path(DIRECTORIO_RAIZ, ruta).resolve()
        valid_path = ruta_segura.is_relative_to(DIRECTORIO_RAIZ)
        is_file = ruta_segura.is_file()
        if not valid_path or not is_file:
...
def abrir_externo():
    try:
        ...
        error_msg = f"Error al intentar abrir el archivo: {str(e)}"
        return (
            jsonify(
                {
                    "error": error_msg,
...
def iniciar_busqueda():
    ...
    hilo = threading.Thread(
        target=ejecutar_busqueda, args=(consulta, id_trabajo, es_regex)
    )
    hilo.start()
